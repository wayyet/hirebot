...
它的唯一职责是：在业务用户进入沙箱、选定模板后，按"资料 → 技能 → 外部"三个阶段进行对话引导，把用户的需求**沉淀到系统 todo 的结构化 `notes` 中**，并在合适的时机发出 `<dispatch>` 信号让系统调起对应的下游 skill。
...
它**会**：
- 维护交接用途 todo 的生命周期（drafting → ... → confirmed）
...
- 本 skill 把 user_summary 复述给用户、等用户确认、把对应 todo 的 `notes.status` 切到 `confirmed`
...
[本 skill 启动 → 角色化开场（基于 soul / identity）]
       ...
       ▼
┌────────────────────────────────────────────────────────────┐
│ 阶段 1：资料  ──→  todo notes (target=ontology_extraction)│
...
└────────────────────────────────────────────────────────────┘
       ...
       ▼
┌────────────────────────────────────────────────────────────┐
│ 阶段 2：技能  ──→  todo notes (target=skill_generation) │
...
└────────────────────────────────────────────────────────────┘
       ...
       ▼
┌────────────────────────────────────────────────────────────┐
│ 阶段 3：外部  ──→  todo notes (target=external_config)  │
...
- "走过" = 该阶段产生过至少一条 `notes.status = confirmed` 的 todo（外部阶段也可以是 `kind: skip` 的 todo）
...
至少 1 份资料被指认归类，且对应 todo 的 `notes` 明确写出"要从中抽什么分类的本体 + 目标"。
...
4. 每收到一份资料 → 实时形成 / 更新 todo `notes`（边传边归类）
...
- todo `notes.payload.auth_kind` 字段只描述凭据形式，不带值
...
| todos | string[] | 是 | 本次交接的系统 todo id 列表，必须存在且 `notes.status` 为 `ready_to_dispatch` 或 `dirty` |
...
| 维度 | 本 skill 维护的交接用途 todo | 诊断 skill 的诊断 todo |
|---|---|---|
