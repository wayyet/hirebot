---
name: diagnosis
<<<<<<<< HEAD:src/HireBot.ApiService/Assets/SystemSkills/employment-coach-conversation/diagnosis/SKILL.md
description: "雇佣教练流程的只读完备性诊断 skill。用于系统层在沙箱初始化、todo 状态变化、dispatch_callback 回传、配置治理修改、阶段出口前，按模板完备性清单评估资料 / 技能 / 外部三阶段还缺什么，并输出带 level 的诊断 todo。不要用于对话引导、生成或修改 todo、执行本体提取、生成技能、配置外部系统、写入 ontology / skills / external，或直接推进阶段。"
========
description: "雇佣教练流程的只读完备性诊断 skill。用于系统层在沙箱初始化、todo 工单状态变化、dispatch_callback 回传、配置治理修改、阶段出口前，按模板完备性清单评估资料 / 技能 / 外部三阶段还缺什么，并输出带 level 的诊断 todo。不要用于对话引导、生成 todo 工单、执行本体提取、生成技能、配置外部系统、写入 ontology / skills / external，或直接推进阶段。"
>>>>>>>> 55b9227ba124696b492fd60ba42dda25d7be1817:src/HireBot.ApiService/Assets/SystemSkills/digital-employee-discovery/skills/diagnosis/SKILL.md
...
========
- todo 工单状态变为 `confirmed` / `needs_review` / `dismissed` 后
>>>>>>>> 55b9227ba124696b492fd60ba42dda25d7be1817:src/HireBot.ApiService/Assets/SystemSkills/digital-employee-discovery/skills/diagnosis/SKILL.md
...
<<<<<<<< HEAD:src/HireBot.ApiService/Assets/SystemSkills/employment-coach-conversation/diagnosis/SKILL.md
- 需要把用户输入整理成下游可执行的系统 todo，这属于 `employment-coach-conversation`
========
- 需要把用户输入整理成下游可执行 todo 工单，这属于 `employment-coach-conversation`
>>>>>>>> 55b9227ba124696b492fd60ba42dda25d7be1817:src/HireBot.ApiService/Assets/SystemSkills/digital-employee-discovery/skills/diagnosis/SKILL.md
...
<<<<<<<< HEAD:src/HireBot.ApiService/Assets/SystemSkills/employment-coach-conversation/diagnosis/SKILL.md
诊断 todo 回答的是“还差什么”。流程 todo 通过 `notes.stage` / `notes.target_skill` 说明目标阶段和下游 skill。两者都是系统 todo，只通过 `notes` 区分用途。
========
诊断 todo 回答的是“还差什么”。todo 工单回答的是“差的部分要交给谁、要带什么去”。两者必须保持分离。
>>>>>>>> 55b9227ba124696b492fd60ba42dda25d7be1817:src/HireBot.ApiService/Assets/SystemSkills/digital-employee-discovery/skills/diagnosis/SKILL.md
...
6. **出口判断**：只有所有必需项 resolved、相关 todo confirmed、且无 blocker 时，才能把 `status` 标为 `pass` 并给出 `ready_for_packaging: true`。
========
1. **读取清单与状态**：先识别模板完备性清单、当前阶段、全部 todo 工单、最新 callback 和沙箱目录快照。
2. **归一化证据**：把 todo 工单按 `stage`、`status`、`category`、`payload` 归类；把下游 artifacts 只作为佐证，不替代 todo 状态。
3. **逐阶段诊断**：按资料、技能、外部三阶段分别判断最低门槛、必需项、推荐项、可选项。
4. **跨阶段一致性检查**：检查配置文件规则是否可能影响已 confirmed todo，特别是判定规则、边界、红线和数据访问范围。
5. **输出诊断报告**：只输出 `diagnostic_report`，不得修改任何文件或 todo 状态。
6. **出口判断**：只有所有必需项 resolved、相关 todo 工单 confirmed、且无 blocker 时，才能把 `status` 标为 `pass` 并给出 `ready_for_packaging: true`。
>>>>>>>> 55b9227ba124696b492fd60ba42dda25d7be1817:src/HireBot.ApiService/Assets/SystemSkills/digital-employee-discovery/skills/diagnosis/SKILL.md
...
========
- 资料阶段：至少 1 条 `material` todo 工单已 `confirmed`
- 技能阶段：至少 1 条 `skill` todo 工单已 `confirmed`，且包含明确的 `skill_name`、`skill_description`、`trigger`、`expected_output`
- 外部阶段：至少 1 条 `external` todo 工单已 `confirmed`，或存在已确认的 `payload.kind: skip`
>>>>>>>> 55b9227ba124696b492fd60ba42dda25d7be1817:src/HireBot.ApiService/Assets/SystemSkills/digital-employee-discovery/skills/diagnosis/SKILL.md
...
========
- `handoff_correlation`: 与已有 todo 工单的关联，而不是改写
- `open_questions`: 输入不足或需要系统层补齐的上下文
- `user_summary`: 可被上层流程转述给业务用户的一两句话

不要输出 `<dispatch>`。不要把诊断 todo 伪装成阶段 todo 工单。不要要求业务用户理解诊断内部字段。
>>>>>>>> 55b9227ba124696b492fd60ba42dda25d7be1817:src/HireBot.ApiService/Assets/SystemSkills/digital-employee-discovery/skills/diagnosis/SKILL.md
