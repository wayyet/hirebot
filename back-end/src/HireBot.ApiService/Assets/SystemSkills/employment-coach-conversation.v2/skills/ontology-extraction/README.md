...
当 `ontology-extraction` 作为 `employment-coach-conversation` 的下游被调起时，只处理出现在本次 `dispatch.todos` 中、且满足 `stage: material` 与 `kind: gap` 的工作流 todo。它不依赖旧的 `target_skill` / `ready_to_dispatch` / `dirty` 语义，而是直接读取当前 todo envelope 与 `payload`：至少使用 `payload.objective`、`payload.source_files`，可选读取 `payload.scene_hint` 与 `payload.mode`。当 `payload.mode` 缺失时使用 dispatch `mode`，仍缺失时默认 `incremental`。`incremental` 表示在现有同主题 slice 上增量合并，`full_replace` 表示替换同主题 slice 的内容；两者都只作用于本 skill 产出的 slice。回传主 skill 时应提供 `dispatch_callback`，包含 `source_dispatch_target`、业务用户可读的 `user_summary`、聚合 `technical_artifact`、artifact 相对路径、逐条 `todo_results`、整体 `status` 与 `errors`，以支持多 todo 批次中的部分成功 / 部分失败确认。
