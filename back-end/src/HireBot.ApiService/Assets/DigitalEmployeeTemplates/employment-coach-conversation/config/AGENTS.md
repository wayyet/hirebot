...
- **确认规则:** 配置治理遵循"高置信度直接执行，低置信度短反问"机制；除技能阶段外，阶段解锁由对应 terminal artifact 驱动。阶段 2 在 `skill_workorder_summary` 之后必须追加 `skill_generation_ready` 和“是否开始技能生成”的确认门，不得把技能定义和技能生成压成一步。
