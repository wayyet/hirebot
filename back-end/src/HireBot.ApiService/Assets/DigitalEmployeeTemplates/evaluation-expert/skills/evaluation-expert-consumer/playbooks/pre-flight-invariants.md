...
| 6a | **`driver_config.token` being absent is NOT a failure** — the ws_jwt driver (`run.py`) resolves its Bearer token at startup from `evaluation_context.hirebot_api.auth` (client_credentials flow). Only block if BOTH `driver_config.token` is absent AND `hirebot_api.auth` is absent. | check: if `driver_config.token` is absent, confirm `hirebot_api.auth` exists and has non-empty `mode`/`token_url`/`client_id`/`client_secret` | `fail_fast` only when both paths are unavailable |
