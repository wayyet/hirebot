...
上面的命令适用于当前目录位于 `ontology-projection 技能根目录内，调用的是支持 `--review-mode` 的真实校验器。

如果从仓库根目录执行：

```powershell
.\scripts\validate-ontology-projection.py .\src\OpenClaw.Gateway\skills\ontology-projection\examples\ready\workflow-contract-projection.json
```
