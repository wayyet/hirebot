...
上面的命令适用于当前目录位于 `ontology-slice-extraction 技能根目录内，调用的是技能目录下的真实校验器；如果需要启发式评审结论，可改用 `--review-mode`。

如果从仓库根目录执行：

```powershell
.\scripts\validate-ontology-slice.py .\src\OpenClaw.Gateway\skills\ontology-slice-extraction\examples\invalid\invalid-sample.json
```
