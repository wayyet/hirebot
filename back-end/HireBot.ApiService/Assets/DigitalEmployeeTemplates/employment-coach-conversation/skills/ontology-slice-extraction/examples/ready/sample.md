...
上面的命令适用于当前目录位于 `ontology-slice-extraction 技能根目录内，调用的是支持 `--review-mode` 的真实校验器。

如果从仓库根目录执行普通结构校验：

```powershell
.\scripts\validate-ontology-slice.py .\src\OpenClaw.Gateway\skills\ontology-slice-extraction\examples\ready\sample.json
```
