...
```json
{
  ...
  "parameters": {
    ...
    "stage": "<stage1_material | stage2_skill | stage3_external | stage4_packaging>",
    "isTerminal": false,
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `kind` | `"data" \| "file"` | `data` 表示结构化数据产物；`file` 表示文件产物（内部实例包，对用户称数字员工包） |
...
| 打包工具成功返回 fileUrl，数字员工包可导入 | `template_package` | `true` | `file` |
...
- **label 必须是业务语言**：例如"已记录 3 份业务资料，等待你确认"，而不是技术字段名；`template_package` 的 label 对用户称"数字员工/数字员工包"，不称"实例包/产物包"
- **对话文案禁用旧词和内部词**：对话回复中禁止出现"dispatch 闭环"、"dispatch 信号"、"handoff 工单"、"ready_to_dispatch"、"dispatch 给下游"、"实例包"、"产物包"、"本体切片"、"ontology"、"projection"、"artifact"、"workorder"等词语
