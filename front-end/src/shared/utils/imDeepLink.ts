 ...
 * 打开与数字员工的 IM 单聊
 */
export function openIMDirect(platform: IMPlatform, botId: string, name: string): void {
  const links: Record<IMPlatform, string> = {
    '飞书': `feishu://open?type=user&openId=${botId}`,
    '钉钉': `dingtalk://dingtalkclient/action/sendmsg?dingtalk_id=${botId}`,
    '企业微信': `wxwork://message/?username=${botId}`,
  }
  ...
  setTimeout(() => {
    if (!document.hidden) {
      const shouldOpenWeb = confirm(`未检测到 ${platform} 客户端，是否在浏览器中打开？`)
      if (shouldOpenWeb) {
        window.open(`https://ncrew.app/im-redirect?bot=${botId}&name=${encodeURIComponent(name)}`, '_blank')
      }
    }
  }, 2000)
}
