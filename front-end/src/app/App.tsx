...
import PrototypeStandalonePage from '@/features/prototype/PrototypeStandalonePage'
    ...
    const mappedEmployees = Array.isArray(employees)
      ? employees
        ...
        .map((item) => ({
          ...
          pendingActions: Array.isArray(item.pendingActions) ? item.pendingActions : [],
          capabilityNames: Array.isArray(item.capabilities)
            ? item.capabilities
              .map((cap: { name?: unknown }) => typeof cap?.name === 'string' ? cap.name : '')
...
function LocalStateMigrationBootstrap() {
  useEffect(() => {
    ...
    api.migration.migrateLocalState(payload)
      .then(() => {
        localStorage.setItem(LOCAL_STATE_MIGRATION_FLAG, '1')
      })
      .catch(() => {
    ...
    <UxOverlayProvider>
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/hiring/:templateId" element={<ProtectedLayout><HiringPage /></ProtectedLayout>} />
          <Route path="/department-employees" element={<ProtectedLayout><DepartmentEmployeesPage /></ProtectedLayout>} />
          <Route path="/team" element={<Navigate to="/department-employees" replace />} />
          <Route path="/my-employees" element={<ProtectedLayout><MyEmployeesPage /></ProtectedLayout>} />
          <Route path="/clone/:id" element={<ProtectedLayout><CloneEmployeePage /></ProtectedLayout>} />
          <Route path="/private-branch/:id" element={<ProtectedLayout><PrivateBranchPage /></ProtectedLayout>} />
          <Route path="/instances/:id" element={<ProtectedLayout><InstanceDetailPage /></ProtectedLayout>} />
          <Route path="/instances/:id/chat" element={<ProtectedLayout><InstanceChatPage /></ProtectedLayout>} />
          <Route path="/instances/:id/im-config" element={<ProtectedLayout><InstanceImConfigPage /></ProtectedLayout>} />
          <Route path="/instances/:id/evaluation" element={<ProtectedLayout><EvaluationPage /></ProtectedLayout>} />
          <Route path="/instances/:id/human-evaluation" element={<ProtectedLayout><HumanEvaluationPage /></ProtectedLayout>} />
          <Route path="/instances/:id/training" element={<ProtectedLayout><TrainingFlowPage /></ProtectedLayout>} />
          <Route path="/instances/:id/review" element={<ProtectedLayout><ReviewPage /></ProtectedLayout>} />
          <Route path="/instances/:id/onboarding" element={<ProtectedLayout><OnboardingPage /></ProtectedLayout>} />
          <Route path="/collaboration" element={<ProtectedLayout><CollaborationPage /></ProtectedLayout>} />
          <Route path="/skill" element={<ProtectedLayout><SkillListPage /></ProtectedLayout>} />
