# 数字员工模板执行链路与 kingcrab skill 调用分析
...
>
> 生成日期：2026-07-01　｜　配套图：
> [雇佣教练模板调用堆栈层次图.svg](雇佣教练模板调用堆栈层次图.svg)、[评估专家模板调用堆栈层次图.svg](评估专家模板调用堆栈层次图.svg)

---

## 0. 一句话结论

这两个目录**本身不是 hirebot 后端的可执行代码**，而是一组"数字员工包"（manifest + config + ontology + 一堆 `SKILL.md`）。hirebot 后端负责把它们**打包上传进 kingcrab（OpenClaw.Gateway）沙箱**并安装成 skill；真正的"执行"发生在 kingcrab 侧的 **Agent Runtime（LLM）**里，通过 `load_skill`（渐进式披露）、`emit_artifact`（产物回传）和 `[Internal downstream trigger: use skill xxx]`（下游触发块）三种机制驱动。

- `employment-coach-conversation` = **单 Agent + 渐进披露 + prompt 内部编排**的"四阶段装配流水线"，产出一个新的数字员工包。
- `evaluation-expert` = Agent 内部再拆 **三段式 multi-agent（Prep / Run / Report）**的"13 步确定性评估流水线"，产出评估报告并回传 hirebot。

---

## 0.5 核心概念速览（读前必读）

> 如果你没接触过"Agent + skill 沙箱"这套东西，直接看后面的链路会很吃力。先花两分钟读这段比喻和名词表，后面就顺了。

### 用一个"机器人代工厂"的比喻理解全局

把整套系统想象成一个**造机器人（数字员工）的代工厂**：
...
1. **沙箱 (sandbox)**：一个隔离的容器环境，Agent 在里面跑。类比 Docker 容器。
...
4. **artifact 契约 (contract)**：Agent 想往传送带放产物时，网关会拿一张白名单（`contracts/artifacts.json`）核对"这个产物的类型/阶段是否合法"。类比：接口的 JSON Schema 校验 / 状态机的合法转移表。

### 一句话看懂三种"调用机制"（后面 3.2 展开）

- `load_skill` = **打开某本手册**（按需加载正文）。→ 有确定性 C# 代码。
- `emit_artifact` = **把产物放上传送带**，触发质检 + 更新前端进度。→ 有确定性 C# 代码。
- `[Internal downstream trigger: use skill xxx]` = 技工给自己写的一张便条"**接下来换个身份干下一道工序**"。→ **没有确定性代码兜底**，就是同一个 LLM 照着 prompt 自觉切换。这是全篇最关键、也最脆弱的一点（见 3.3 / 4.2）。

> **一句话记住全局**：hirebot 只做"搭台"（建沙箱 + 传包 + 入库），真正的"唱戏"（装配 / 评估）全在 kingcrab 沙箱里由**一个 LLM** 边读手册边执行；kingcrab 只对"加载手册、放产物、质检产物"这三件事提供确定性保障，**"下一步该干嘛"靠 LLM 遵守 prompt**。

---

## 1. 两个模板的目录与角色

### 1.1 employment-coach-conversation（生产链）
...
**6 个下游 skill 的 producer-consumer 管道**（见 `references/downstream-handoff-registry.md`）：

```
R1 ontology-slice-extraction  ──写──►  ontology/*.slice.json
        │
R2 ontology-projection        ──写──►  ontology/projections/<slug>/*.projection.json
        │
R3 skill-generation           ──读 projection──►  skills/<slug>/（SKILL.md + 4 视图 contract）
        │
R4 packaging-test-cases（可选）──写──►  testcases/evaluation-test-cases.json
        │
R5 completeness-review（可选）──►  P0/P1/P2 审查报告
        │
R6 打包 ──► template_package(kind:file) ──► 前端 auto-importPackage
```

### 1.2 evaluation-expert（评估链）

`manifest.json`：`entry_skill = skills/evaluation-expert-consumer`，只有 1 个 skill。
[evaluation-expert-consumer/SKILL.md](../back-end/HireBot.ApiService/Assets/DigitalEmployeeTemplates/evaluation-expert/skills/evaluation-expert-consumer/SKILL.md) 定义了**与模板无关的 13 步确定性评估工作流**，角色差异全部通过 6 个热插拔数据层（`metrics/`、`test-cases/`、`runtime-drivers/`、`simulators/`、`role-catalog/`、`employees/`）承载。

---

## 2. 执行链路（问题 #1）

### 2.1 生产链：employment-coach-conversation

#### 阶段 A — hirebot 后端准备沙箱（确定性 C# 代码）
...
#### 阶段 B — 会话运行（前端 ↔ kingcrab，WebSocket）
...
Agent 会话初始化（SKILL.md「会话初始化」章节）：读首轮 `[FILE_URL:/workspace/<slug>-<ts>]` → 锁定 `workspace_root`（会话级常量）→ 读 `manifest.json` 定 `template_slug` → 规范化 `config/`。

随后四阶段推进，每个阶段都在重复同一个机器循环：
...
### 2.2 评估链：evaluation-expert

#### 阶段 A — hirebot 后端准备评估沙箱
...
#### 阶段 B — 评估运行（kingcrab Agent，内部 multi-agent）
...
## 3. 最终如何调用 kingcrab 的 skill（问题 #2）

需要区分**两层"调用"**：

### 3.1 安装层：模板包 → kingcrab skill
...
### 3.2 运行层：三种机器机制驱动 skill

| 机制 | kingcrab 实现 | 作用 |
|---|---|---|
| `load_skill` | `LoadSkillTool` | **渐进式披露**：系统提示只放 skill 元数据索引；模型判断需要某 skill 时才把它的 `SKILL.md` 正文 + 资源清单 + artifact 契约拉进上下文 |
| `emit_artifact` | `EmitArtifactTool` + `SkillArtifactRuntime` | 把阶段产物（`kind:data`）或文件（`kind:file`）经 WebSocket envelope 即时推前端；`SkillArtifactRuntime` 按 `contracts/artifacts.json` **白名单校验** type/stage，标记 terminal，产出 `SkillStageGateEvent` 阶段门 |
| `[Internal downstream trigger: use skill xxx]` | **无 C# 实现**（仅 prompt 约定） | 主 skill 输出该文本块，同一 LLM 据此"切换"到下游 skill 指令继续执行 |

### 3.3 关键事实：下游触发不是确定性代码
...
3. **跨 skill 契约偏隐式**：`workspace_root` / `template_slug` / `items[]` 靠 artifact `data` 透传，但 `SkillArtifactRuntime` 只校验 type/stage，**不校验 data 内部字段**；data 完整性主要靠 prompt 约束 + LLM 自查（R3 的字段来源验证清单有 6~8 项要模型逐条核对）。
...
> 详细分层调用堆栈见：[雇佣教练模板调用堆栈层次图.svg](雇佣教练模板调用堆栈层次图.svg)、[评估专家模板调用堆栈层次图.svg](评估专家模板调用堆栈层次图.svg)
